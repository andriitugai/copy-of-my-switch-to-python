...
negative_sum = sum([float(num) for num in list_1 if is_number(num) and float(num) < 0])
...
dict_2 = {
    k: v["skills"][-2:] if v["role"] == "jedi" else v["role"] for k, v in dict_1.items()
}
...
def gen_3():
    yield from gen_1(sort_by_height_desc)
    yield from gen_2
